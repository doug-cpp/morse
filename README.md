...
Por ser uma aplicação simples, ao usar a forma gratuita do *Heroku*, existe um comportamnto similar à hibernação das funções lambda, gerando lentidão no primeiro acesso.[^lambda]
...
Devemos seguir o padrão de mercado que impõe a utilização de conexões seguras e criptorafadas para tráfego de dados (Https), onde é necessário instalar certificados [`TSL ou SSL`](https://pt.wikipedia.org/wiki/Transport_Layer_Security) em nosso servidor.
